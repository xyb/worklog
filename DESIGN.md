...
- **Decision rule — where does a fact go: `prop` vs `metric` vs `log`?** Sort by *"will I filter / group / aggregate over it across nodes?"* and by *cardinality*:
  ...
  - **Structured plan props are `plan.*`, derived from a reserved-tag log** — a `goal`/`top5` log (goal/summary/overview/top5 are `log` rows with a reserved `tag`) names the task/project nodes it plans in free text (`wl goal "ship #42 + draft #7"`); on each write `_set_typed_log` extracts those `#ids` into a `plan.<tag>` prop (`plan.goal` / `plan.top5`, comma-separated id list) on the same node. So the plan↔node link is **queryable** (`wl ls --prop plan.goal` = which days targeted what) rather than buried in prose, while the prose itself stays the source of truth (the prop tracks the *latest* text: overwrite on rewrite, cleared when no node is named). `PR#`/`LUM-` runs and dead ids are skipped (`_extract_node_refs`). `plan.*` is **system-managed** — `_reserved_prop_hint` rejects a hand `wl set` so the prop and the goal text can't disagree; the write hook bypasses that guard via a direct `_db.upsert`. Same "reuse prop + namespace, no new table" rationale as relations.
