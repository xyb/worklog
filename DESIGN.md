...
- `wl show -o/--output {text,json}` — the first **structured-output** surface. `text` (default) is the rich rendering; `json` prints `_node_to_dict(con, n)` via `json.dumps(ensure_ascii=False, indent=2)` (plain `print`, never `out()` — machine output isn't styled/wrapped): the full node + relations (tags / ancestors / props / links / schedule / children / logs / metrics / clock), no timeline elision, single id → object, several → array. Field names mirror the DB columns (stable as an API contract); `*_at` are UTC instants verbatim, `*_date` local calendar days. Scoped to `show` for now; the plan is to promote `-o` to a global flag with per-command serializers (see the backlog). Models the k8s `-o json` / `gh --json` pattern: let an AI/script pull an exact field instead of parsing the text view (G3).
