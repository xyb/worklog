...
- `-o/--output {text,json}` — the **structured-output** surface, a shared `output_parent` parser on the commands that serialize: `show` / `ls` / `logs` / `projects` / `day` / `tree` / `summary` / `hours` (a command without it rejects `-o` rather than silently printing text). `text` (default) is the rich rendering; `json` prints via `json.dumps(ensure_ascii=False, indent=2)` (plain `print`, never `out()` — machine output isn't styled/wrapped). `jsonl` is the same data reshaped for line-oriented pipes: a top-level array streams one compact object per line (empty array → no output), any other payload a single line — a `JSONLFormatter(JSONFormatter)` overriding only `emit()`, so out()-suppression + RFC 9457 errors are inherited. `toon` emits [TOON](https://github.com/toon-format/spec) (Token-Oriented Object Notation, spec v3.3) — the same data losslessly in a compact indentation-based format (~40% fewer LLM tokens; a uniform primitive-object array becomes a table). The encoder is `worklog/toon.py` — hand-rolled, zero-dependency (G3: borrow the technique, not the library); `TOONFormatter(JSONFormatter)` just swaps `emit()`. All formatters register through the same `register_formatter()` API a plugin would use. `show` → `_node_to_dict` (full node + relations: tags / ancestors / props / links / schedule / children / logs / metrics / clock, no elision; single id → object, several → array). `ls` → array of `_node_summary_dict` (compact: identity + filter/sort fields + tags; filters apply, the display 20-cap is skipped, explicit `--limit`/`--top` honored). `logs` → array of log rows. `projects` → array of project summaries + `counts` (done/doing/pending/total) + `latest_activity`. `day` → `{date, weekday, nature, node_id, goal, goal_targets ([{id,title,status}], the goal's structured target nodes in priority order), goal_progress, summary, summary_at, week_goal, month_goal (the day node's reserved-tag logs, flattened in — only present keys), tasks[{…, logs, planned, clock_min}], clock_min_total}`. `tree` → the nested structural subtree (`--root` or top-level forest, each node + `children[]`, depth-bounded; ignores the text `--by`/filter sugar). `summary` → `{since, until, totals, by_direction, done[], pending[]}`. `hours` → `{since, until, by, total_min, groups[{id,title,min,pct}]}` — where time went, reconstructed from the log stream (each adjacent-log interval credited to the earlier log's node when the gap ≤ 60 min, else dropped as a break), grouped by project / task / day; measures log-activity time, not human presence. `hours --spans` → `{since, until, total_min, spans[{start,end,min,node_id,title,project_id,project}]}` — the same reconstruction as a chronological list of continuous same-node spans (a node switch or break ends a span), `start`/`end` UTC instants, for overlaying against other presence sources (the provider role in the presence-timeline design). Empty result → `[]`. Field names mirror the DB columns (stable as an API contract); `*_at` are UTC instants verbatim, `*_date` local calendar days. `type` is the single representative token (`node_type_from_props`, precedence para > date > habit > meetlog > custom > task) — a scalar everywhere (show / ls / children / ancestors / find / focus), not the raw facet map, so a uniform node array stays primitive and encodes as a TOON table; the full orthogonal `type.*` classification still rides in the `props` map. Models the k8s `-o json` / `gh --json` pattern: let an AI/script pull an exact field instead of parsing the text view (G3).
...
- **`timeutil` is the ONLY module that touches a clock or a zone** — enforced by `tests/test_time_lint.py`, not by review. Nothing else may call `datetime.now()` / `date.today()` (read the clock: `utc_now` / `local_now` / `today` / `today_date` / `now_dt`, which honor `$WORKLOG_TZ`) nor `datetime.strptime` / `datetime.fromisoformat` on a stored stamp (parse it: `parse_ts`, or the arithmetic built on it — `elapsed_sec` / `age_min` / `shift_ts` / `days_ago` / `days_ahead`; `local_input_to_utc` parses a user-typed `--at`, anchored either on now or on an existing log's own stamp). Every hand-rolled copy is a fresh chance to compare a UTC value against a local one, and both flavors of that bug shipped before the lint existed: a staleness cutoff derived from the UTC date but compared against a local activity date (off by a day for the hours the local date leads UTC), and an age formatter that hand-parsed `%Y-%m-%d %H:%M:%S` and so returned "—" on exactly the legacy date-only rows that were oldest. `parse_ts` reads a date-only stamp as that day's **local** midnight — the same rule `local_day_sql`'s CASE applies, so SQL and Python can never disagree about which day an old row falls on. `elapsed_sec` is deliberately **signed**: a negative span means an end landed before its start (`wl clock edit`), a user error the caller must reject — flooring is a policy the call site owns (`max(60, …)`), and a primitive that clamped would swallow the error. Still allowed anywhere: `date.fromisoformat(s)` to validate a plain calendar date (no zone involved) and `timedelta` arithmetic on an already-local date.
...
- **`sched` table**: `(id, node_id, on_date, recurrence, created_at)`. `on_date` and `recurrence` are mutually exclusive; a task can have multiple rows (multiple days, or one-off + recurring together).
  - `on_date`: one-off scheduling to a specific `YYYY-MM-DD`
  - `recurrence`: how the schedule repeats — `daily` / `weekly:Mon,Wed` / `monthly:5|-1` / `quarterly:M-D|-1` / `yearly:MM-DD|-1` (`_normalize_recurrence` validates; weekdays normalize to `Mon..Sun`).
  - **This is a private DSL, not RFC 5545 RRULE**, despite the field having been called `rrule` until migration 0014. Two of the forms have no standard reading: `quarterly:` is not an RFC frequency at all (the standard spells a quarter `FREQ=MONTHLY;INTERVAL=3`), and a leading `-1` means *Sunday* in `weekly:-1` where the standard's `-1` prefix means *the last one of the period*. The names were changed because a reader who knows RRULE was being misled by the old one; the syntax stays terse on purpose (G3 — these strings are typed and read constantly).
  - **Every rule is a pure predicate over the target date** — `_sched_fires` answers "does this fire on `target`?" from the date alone, with no anchor and no history. That is what keeps recurrence stateless and idempotent. Note the consequence: an *interval* rule ("every other Wednesday") is **not expressible** in this model, because an interval needs a phase origin. Adding one means giving a recurrence row an anchor date (the standard's `DTSTART`), not just a new keyword. An ISO-week-parity trick is not a substitute — 53-week years (2026 is one) flip the parity across the year boundary and drift the phase by a week.
- **`_sched_fires(on_date, recurrence, target)`**: does this row fire on `target`. `_scheduled_node_ids(con, target)` collects nodes hit on a given day.
...
- **Stopping a recurrence — `wl sched stop <id> [date] [--rule R]`**: ends a recurrence at an **inclusive** end date (default today), keeping history. Encoded as a `;until=YYYY-MM-DD` suffix on the recurrence string (`weekly:Wed;until=2026-06-30`), *not* a new column: `_split_until(recurrence)` strips it, and `_sched_fires` returns False for `target > until` — so every recurrence consumer (`_next_sched_fire`, `wl show`) stays until-aware for free by splitting first. Contrast with `--clear`/`rm`, which delete the row and erase the rule from *past* days too. A node with several recurrences stops all unless `--rule` names one. `wl show` renders it `recur weekly:Wed (stopped 2026-06-30)`. `_recurrence_is_active(recurrence, today)` (no until, or until >= today) is the single "is this a live recurrence" test, shared by the `wl done` recurring-warning, `wl show`'s `last check-in:` line, and `wl ls --not-checked-in`.
...
- **`wl agent` observability — the forward view and the reverse one.** `wl agent ls` lists the live session↔node bindings; each row carries a **read-time derived** activity column — how long since the node was last logged on (`_age_short`), plus 💤 once that exceeds `--stale-days` (default `AGENT_STALE_DAYS` = 3). Nothing is stored: an activity flag is *time-varying state*, and a persisted one is wrong the moment the clock ticks. The bind's own history log is **excluded** from that "last worked" time — counting it would make a freshly-bound node look active forever, which is precisely the claim this view exists to test. `wl agent gaps` is the reverse and the one a forward listing structurally cannot answer ("what has NOBODY on it?"): important work with no binding, from two sources — a still-open target of **today's goal** (settled ones drop out via `_target_settled`, so a ticked recurring target isn't flagged) and a **DOING P0** (declared in flight, nothing pushing it). Two exclusions, both learned from real data: a *recurring* item never leaves DOING (`wl tick` doesn't move the status) and is kept up by check-ins rather than a session, so it's skipped; and a merely-open P0 is **not** a gap — priority is a standing ranking, not a claim of being in flight, and counting them sweeps in nearly every open P0 at once — an alert that always fires and therefore never informs.
