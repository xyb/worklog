...
- **Query decomposition — prefer simple reads + Python compose over complex SQL.** The standing direction, consistent with G3 and with "no foreign keys" + "the time model derives membership in Python from the date, not a parent chain": decompose a complex read into simple single-table SELECTs composed in Python — each wrapped as a small function, with a command-scoped cache where one classification is consulted many times — rather than a JOIN / EXISTS-subquery / aggregate in one SQL statement. **Why:** the DB is small (low thousands of rows, ~2 MB), so a full-table read + Python compose is sub-millisecond and measures *faster* than the subquery SQL it replaces (the `workitem_sql` filter — six EXISTS subqueries — is 1.90 ms/call; reading all nodes and classifying in Python via the pure `node_type_from_props` is 1.59 ms, identical result set). Complex SQL is also the part hardest to maintain, profile, and refactor (by a human or an AI), so minimizing it is a maintainability win, not a perf cost. The `db_table` "NOT an ORM — complex reads stay explicit SQL" stance narrows accordingly: keep raw SQL only where decomposition genuinely can't help (e.g. GROUP BY vocab stats) or for projections; anything classifiable by props moves to a Python index. Simple single-table reads that were still hand-written SQL (`semantic.py` keyword LIKE, count(*)) have been aligned to `_db.query` / `Node.count` — `cmd_find` was already the template.
- **Type classification is a Python snapshot, not EXISTS subqueries.** A node's type derives from its `type.*` props by the pure `node_type_from_props(props)` (precedence para > date > habit > meetlog > custom > task). The SQL forms that re-encoded this as EXISTS subqueries (`workitem_sql`, `nodes_with_type`, `time_node_by_period`, per-node `node_type`) are being moved onto a command-scoped index: read all live `type.*` props once into `{node_id: {key: value}}`, then classify by dict lookup. The index is a **snapshot** — built at the top of a read command, reused across every classification in that command, not held across writes.
