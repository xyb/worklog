...
- **No links to the maintainer's local files.** Never reference a private memory (`feedback_…`), a `~/.claude/…` path, or any machine-local file from anything in this repo (SKILL.md, help, comments, changelog). Those don't exist for other readers. Keep guidance self-contained.
