...
- **DRY (Don't Repeat Yourself).** The codebase has a small set of single-source helpers — `_status_marker`, `_node_line`, `out`/`_c`, `_resolve_window`, `_resolve_db_path`, `_project_members`, `_node_clock_min`, `_collect_descendants` (see DESIGN §12 for the full list). **All time/date handling lives in `timeutil`** and nowhere else — never call `datetime.now()` / `date.today()` or hand-parse a stored stamp with `strptime`/`fromisoformat`; use `utc_now`/`today`/`parse_ts`/`elapsed_sec`/`age_min`/`days_ago`/… (`tests/test_time_lint.py` fails the build otherwise — mixing a UTC value with a local one has shipped real off-by-a-day bugs). New code reuses them; re-implementing one of them in a fresh form is a review block. Same rule for docs: install / dev / release info is in CONTRIBUTING.md and only there — link to it, don't restate it.
...
- **Spell new names out.** New identifiers use whole words — `token` not `tok`, `cursor` not `cur`, `config` not `cfg`, `value` not `val`, `normalize` not `norm`. Short forms are fine when the short form is the word readers know: `id`, `url`, `db`, `ok`, `args`, `kwargs`, plus a loop index (`i`) whose scope fits on one screen. **Two established exceptions, and they are not up for local debate**: the DB handle is `con` (fixed by the `cmd_<name>(args, con)` handler contract above and by `_node_line(con, …)`) and a node id argument is `nid` — `con` alone occurs ~2700 times across `src/` + `tests/` and `nid` ~700, so a file-by-file change would only make the codebase less consistent, not more. Changing them is a whole-repo sweep of its own; until then, match the surrounding code. Two further rules apply everywhere, both from real confusion here: **never let two names in one function differ by only a letter or two** (the recurrence matcher had `d` for the target date's day next to `dd` for the day the rule asks for — now `day` / `rule_day`), and **never name a thing after a standard it does not implement** (see the `recurrence` field, called `rrule` until it was renamed, while being a private DSL and not RFC 5545 RRULE). A sweeping rename lands as its own commit, never mixed into a behavior change.
