...
- **Spell names out; abbreviations are a review block.** Identifiers use whole words — `connection` not `con`, `node_id` not `nid`, `token` not `tok`, `cursor` not `cur`, `config` not `cfg`, `normalize` not `norm`. The allowed short forms are the ones that are words in their own right to any reader: `id`, `url`, `db`, `ok`, `args`, `kwargs`, plus a loop index (`i`) whose whole scope fits on one screen. Two further rules, both from real confusion in this codebase: **never let two names in one function differ by only a letter or two** (`d` vs `dd`, `m` vs `mm` — say `target_day` / `rule_day`), and **never name a thing after a standard it does not implement** (see the `recurrence` field, which spent its first months called `rrule` while being a private DSL, not RFC 5545 RRULE). Typing is cheap and happens once; reading happens forever. Renaming existing abbreviations is welcome as its own commit, never mixed into a behavior change.
