...
| Record a number / measurement / check-in | `wl metric add <id> glucose 5.4 --unit mmol/L` / `wl metric ls <id>` (omit the node + `--tag X` → find that tag across **all** nodes, e.g. before renaming) / `wl metric edit #M7` / `wl metric rm #M7`; inline: `wl log <id> "..." --metric 'pullups 8'`. **Any recurring item's done-today = a `checkin` metric** (`wl tick`/`wl checkin`) — habit *or* recurring task alike; `wl day` renders `[x]` for it that day and the node stays recurring (status untouched). `wl show` shows its `last check-in: <date>`; `wl ls --not-checked-in N` lists recurring items not checked in within N days (or never). Detail → `references/features.md` |
