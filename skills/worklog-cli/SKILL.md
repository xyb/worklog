...
- **Update** — `~ #id` then indented field-ops: `status` / `priority` / `title` / `parent N` (reparent, cycle-checked) / `scheduled` / `deadline` / `+tag` / `-tag` / `+link` / `-link` / `prop k=v` / `-prop` / `goal <text>` / `summary <text>` / `+metric tag [value] [unit]`. Only declared fields change. **Relations go through `prop`** — `prop relation.block=N` (also `relation.split` / `relation.related`); the reverse (`=blocked-by` etc.) derives at read time. `goal`/`summary` write the reserved-tag log (declarative `wl goal set` / `wl recap`); `+metric` writes a node-level datapoint (declarative `wl metric` / `wl log --metric`; a bare tag = a marker like `wl tick`).
