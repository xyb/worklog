...
| Search by keyword vs by meaning | `wl find <kw>` (exact substring across title/body/log/tag — primary, no setup) · `wl query "<text>"` (**semantic**: nearest nodes by meaning, finds paraphrases `find` misses; `--limit N` / `--threshold T` / `-o json`). `query` needs the **`semantic` extra** (`pip install 'pyworklog[semantic]'`) + a one-time `wl reindex` (embeds all nodes via the configured OpenAI-compatible server; backend in `wl config`). Use `find` for exact words/names, `query` for concepts |
