...
| Bind / show which task **this agent session** is on | `wl agent <id>` binds the current Claude Code session to a node (session id from `$WL_SESSION_ID` / `$CLAUDE_CODE_SESSION_ID`, fails closed if neither); `wl agent` shows it · `wl agent ls` lists all · `wl agent rm` unbinds. Stored as the `agent_session.claude` prop = **live pointer** (one session → one node, no new table). `wl agent <id> --record` also leaves a permanent **history** mark (one log + an `agent_session` metric with the full sid) — recover past sessions a node was worked under with `wl metric ls <id> --tag agent_session --all`. A status line + a `UserPromptSubmit` hook can surface the live binding — the scripts ship with this skill under `integrations/` (jq-free; use `wl agent context [--hook]`), and **`references/setup.md` has the check-&-install steps** the assistant runs on request (`wl help agent` is the human walkthrough) |
