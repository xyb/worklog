...
### Added
- **`wl hours`** — where time went, reconstructed from the log stream: each adjacent-log interval (gap ≤ 60 min) is credited to the earlier log's node, then grouped by project / task / day. A gap over 60 min is a break (lunch / overnight) and is dropped whole. Measures log-activity time (human or agent), not human presence — an honest wl-only replacement for the parallel-agent-inflated `clock`. `--spans` lists the raw continuous activity spans (start–end per node, chronological) for overlaying against other presence signals. `-o json/jsonl/toon` supported.
- **`-o toon` output** — [TOON](https://github.com/toon-format/spec) (Token-Oriented Object Notation): the same data as `-o json`, losslessly, in a compact indentation format that costs an LLM ~40% fewer tokens (a uniform object array becomes a table). Hand-rolled zero-dependency encoder. Works on every command that already takes `-o json`.
- **`-o jsonl` output** — a top-level array streams one compact JSON object per line (empty → no output); other payloads stay one line. Same data as `-o json`, shaped for `jq -c` / `grep` / line-oriented analyzers. Works on every command that already takes `-o json`.
- **Overview commands auto-brief inside an agent session** — `day` / `ls` / `projects` / `active` / `summary` default to `-q` when a session id is present (17–93% smaller output). The flip prints a notice; `--no-brief` opts back out. `show` / `logs` excluded: brief drops the very detail you ran them for.
- **`wl agent gaps`** — the reverse of `wl agent ls`: important work with NO session pushing it (a still-open target of today's goal, or a DOING P0). The forward listing can only ever show what IS bound, so it can't surface a task nobody is on.
- **`wl agent ls` shows how long since each node was last worked** (`52m` / `3d`) + 💤 past `--stale-days` (default 3) — a bound session whose work has stopped. `--no-activity` hides the column.

### Changed
- **`-o json` `type` is now a scalar token, not a facet object.** Every node payload (`show` / `ls` / `children` / `ancestors` / `day` / `tree`) emits `type` as the single representative token (`"task"` / `"project"` / `"day"` / …) — aligning with `find` / `focus`, which already did. Was the orthogonal facet map (a bare task → `{}`, a project → `{"para":"project"}`). This keeps uniform node rows primitive so they encode as a compact TOON table; the full `type.*` classification still rides in `show`'s `props`.
- **All time/date handling is consolidated in `timeutil`, enforced by a lint test.** Nothing outside it may read the clock (`datetime.now()` / `date.today()`) or hand-parse a stored stamp (`strptime` / `fromisoformat`) — new primitives `parse_ts` / `now_dt` / `elapsed_sec` / `age_min` / `shift_ts` / `days_ago` / `days_ahead` / `local_input_to_utc` replace ~26 scattered copies. Mixing a UTC value with a local one was a live source of off-by-a-day bugs; `tests/test_time_lint.py` now fails the build on a new one.
- **`wl agent ls --by active` no longer counts the bind itself as activity** — binding wrote a history log, which made a freshly-bound node look worked-on forever.

### Fixed
- **A recurring goal target settles by check-in, not status** — a habit / recurrence never reaches DONE, so `wl tick` now closes it out in the goal header (`[x]` + `[done/total]`) instead of pinning it at `[ ]` forever. `wl day -o json` target rows gain a `done` field.
