...
### Added
- **`=backrels` in `wl show`** — the `relations:` block now ends with a derived `=backrels` row: ids of other nodes whose text references this one (`#id`/`WL#id` mentions; "what links here"/backlinks). It's machine-computed (not a stored prop), so it's marked with a leading `=` + italic-dim to set it apart from the real relations above — wl's convention for a derived/read-only field. In `-o json` it's the `backrels` array. (`PR#`/`LUM-` runs don't count, so PR/Linear refs aren't mistaken for node refs.) See `wl help relation`.
