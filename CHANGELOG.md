...
- **`wl agent` records *which* agent runtime** (claude / cursor / codex / …), not just the session id — resolved from `--agent NAME` / `$WL_AGENT` (default `claude`) and stored as the metric `note` + the `agent_session.<agent>` prop-key suffix, so the bind history shows what worked the node. (Was hardcoded `claude`.)
