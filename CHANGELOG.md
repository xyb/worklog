...
### Added
- **Structured plan props — `wl goal`/`top5` extract their `#ids` into a queryable `plan.*` prop**. Writing a goal/top5 reserved-tag log whose text names task/project nodes (`wl goal "ship #42 + draft #7"`) now also (re)writes a `plan.goal` / `plan.top5` prop on that node with those ids, so the plan↔node link is queryable (`wl ls --prop plan.goal` → which days targeted what) instead of living only in free text. The prop tracks the latest goal text (overwrite; cleared when the text names no node); `PR#`/`LUM-` runs and non-existent ids are ignored. `plan.*` is system-managed — set it by writing the goal, not `wl set` (rejected).
