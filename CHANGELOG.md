...
- **`--help` now wraps the raw epilog/description with a hanging indent + caps its width.**
  ...
  the terminal edge with no indent on a narrow window. It now wraps argparse-style — a two-column
  `label   description` row hangs its continuations under the description column, bullets under
  their text — measured on *visible* width so a markdown span (`` `wl day` ``) is never split
  across the boundary. A `_WlHelpFormatter` caps argparse's own wrap width on wide terminals
  (`render.HELP_MAX_WIDTH`, 100 cols), and the epilog wrapper uses the same `render.help_width()`,
  so the two line up at one width.
