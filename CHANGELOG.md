...
- **`wl agent context`** — prints the current session's binding as a machine line `<id>\t<title>`
  (empty if unbound), or with `--hook` the ready-to-emit Claude Code `UserPromptSubmit` JSON
  (`wl` does the escaping, so the hook needs no `jq`). Lets a hook fetch the binding through `wl`
  (one owned query) instead of hand-writing SQL, cache it, and re-inject only when it changes:
  `wl agent set` / `rm` invalidate a per-session cache under `$XDG_STATE_HOME/worklog/agent/`, so
  the hook's common path touches neither the DB nor `wl`. The skill now **ships the integrations**
  (`integrations/wl-session-context.sh` + `integrations/statusline-wl.sh`, jq-free, depend only on
  `wl`); `wl help agent` + the skill's `setup.md` cover install.

### Changed
- **`wl agent <id>` records the session→node history by default now** (was opt-in via `--record`
  in 0.7.0). Auto-binds — a session that binds itself without passing a flag — now capture the
  lineage too; `--no-record` opts out for a pointer-only bind. Still one row per *binding* (not
  per write), and rebinding the same pair doesn't duplicate it.
