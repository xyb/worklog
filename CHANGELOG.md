...
### Added
- **`wl query` / `wl reindex` — semantic search.** Embeds nodes via an OpenAI-compatible `/v1/embeddings` server into a sidecar LanceDB; `wl query` ranks by similarity (`--limit`/`--threshold`/`-o json`). Configurable backend (`wl config`); optional `semantic` extra, lazily imported.
