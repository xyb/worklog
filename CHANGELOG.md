...
### Breaking
- **`sched.rrule` is now `sched.recurrence`** (migration 0014, values untouched). The column was named after RFC 5545's RRULE but never implemented it — `quarterly:` is not an RFC frequency, and `weekly:-1` means Sunday where the standard's `-1` means "the last one". Anything reading the column directly (external SQL, `-o json` consumers of `sched ls`) must use the new name; the `--recur` flag and every rule string are unchanged. Internal helpers renamed to match (`_normalize_recurrence` / `_recurrence_is_active` / `_format_recurrence`).
