...
- **`--kind` filter removed from `ls`/`tree`/`day`/`logs`/`agenda`/`find` (and `wl checkin`).** Filter by role with `--para area|project|task` (exact, matches the `type.para` prop); filter other classifications with `--prop type.<x>` (e.g. `--prop type.meetlog`). `wl find` gains `--para`; `wl checkin` keeps `--all-kinds` (habit + task + meetlog). The JSON `kind` field (`wl show -o json`) stays, now derived.
