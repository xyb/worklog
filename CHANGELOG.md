...
### Breaking
- **`rrule` → `recurrence` everywhere it is readable** (migration 0014; rule strings and the `--recur` flag unchanged) — the `sched.rrule` column, `wl sched ls -o json` `rows[].rrule`, `wl show -o json` `schedule.rrules` (now `.recurrences`), and `wl sched --recur -o json` `ops[].type` (`"rrule"` → `"recurrence"`). The old name claimed RFC 5545 RRULE, which this DSL is not: `quarterly:` is no RFC frequency and `weekly:-1` means Sunday, not "the last one".
