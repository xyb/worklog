...
### Added
- **`wl doctor`** — scan the node graph for inconsistencies no foreign key prevents (dangling `parent_id`, parent cycles, orphaned spoke rows, `relation.*` refs to dead nodes, one-sided relations). Read-only; reports by kind + node. `-o json` for structured output.
