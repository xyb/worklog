...
- **JSON output for all remaining commands** — `find`, `focus`, `ancestors`, `descendants`, `agenda`, `changes`, `goal` (read + ls), `prop ls`, `log ls`, `log show`, `link ls`, `tag ls`, `sched ls`, `metric ls`, `add`, `log` (write), `done`, `cancel`, `reopen`, `wait`, `defer`, `start`, `stop`. Write commands suppress their human-readable confirmation lines in json mode so stdout stays machine-parseable.
