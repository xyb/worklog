...
### Breaking
- **`kind` retired — node classification now lives in the `type.*` prop namespace.** A node's kind is no longer a stored column; it is derived from `type.para` (area/project/task), `type.date` (time level), `type.habit`, `type.meetlog`. Migration **0011** backfills `type.*`/`date.*` for every node, verifies a lossless round-trip, then **drops the `node.kind` column** — it runs automatically on first use under an exclusive write lock, with a pre- and post-migration backup.
- **`-k/--kind` removed from `wl add` / `wl node edit`.** Set the responsibility role with `--para area|project` (a bare add is a task); set other classifications with `--prop type.habit` / `--prop type.meetlog` / `--prop type.date=day`. Reclassify later with `wl node edit --para <role>` or `wl set type.<x>`.
- **`--kind` filter removed from `ls`/`tree`/`day`/`logs`/`agenda`/`find` (and `wl checkin`).** Filter by role with `--para area|project|task` (exact, matches the `type.para` prop); filter other classifications with `--prop type.<x>` (e.g. `--prop type.meetlog`). `wl find` gains `--para`; `wl checkin` keeps `--all-kinds` (habit + task + meetlog).
- **JSON output drops the collapsed `kind` token for an orthogonal `type` facet.** Every `-o json` payload (`show`/`ls`/`tree`/`day`/`projects`/`summary`) now carries a `type` object (the `type.*` facets, prefix stripped, e.g. `{"para":"project","habit":true}`; a bare task → `{}`) instead of a single derived `kind` string.
- **`wl kinds` → `wl types`.** Lists the raw `type.*`/`date.*` classification props in use, grouped by key (no auto-collapse to one `kind`); a node appears under every facet it carries.
- **`wl import` / `wl apply` drop the `kind` field/token — classify with the real `type.*` props.** JSON import: `{"props": {"type.para": "project"}}` (or `type.date`/`type.habit`/`type.meetlog`; a bare node is a plain task). Apply: an `@prop type.para=project` sub-line, not a `[project]` token on the node line.
...
- **Background auto-reindex** — after any write, a detached single-flight worker (`wl reindex --auto`) incrementally re-embeds new/changed nodes and loops until the index is clean, so `wl query` stays fresh without a manual reindex. On by default; disable with `[index] auto_reindex = false` (config.ini) or `WORKLOG_AUTO_REINDEX=0`.
