...
### Breaking
- **`rrule` → `recurrence` everywhere it is readable** (migration 0014; rule strings and the `--recur` flag unchanged) — the `sched.rrule` column, `wl sched ls -o json` `rows[].rrule`, `wl sched <id> <when>|--recur -o json` `schedule[].rrule`, `wl show -o json` `schedule.rrules` (now `.recurrences`), and that same write path's `ops[].type` (`"rrule"` → `"recurrence"`). The old name claimed RFC 5545 RRULE, which this DSL is not: `quarterly:` is no RFC frequency and `weekly:-1` means Sunday, not "the last one".

### Fixed
- **`$WORKLOG_COLOR=always` now forces color through a pipe**, matching the `--color always` flag — the env path resolved color on but left `force_terminal` unset, so a non-TTY pipe silently produced no ANSI.

## [0.11.0] - 2026-07-16

### Breaking
- **`-o json` `type` is now a scalar token, not a facet object** — every node payload (`show` / `ls` / `children` / `ancestors` / `day` / `tree`) emits `type` as the single representative token (`"task"` / `"project"` / …) like `find` / `focus` already did; was the facet map (bare task `{}`, project `{"para":"project"}`). Keeps uniform rows primitive so they encode as a TOON table; the full `type.*` classification still rides in `show`'s `props`.

### Added
- **`wl hours`** — where time went, reconstructed from the log stream (an adjacent-log gap ≤ 60 min credits the earlier node; a larger gap is a break, dropped), grouped by project / task / day. Log-activity time (human or agent), not presence — a wl-only replacement for the parallel-agent-inflated `clock`. `--spans` lists the raw continuous spans (start–end per node) to overlay against other presence signals. `-o json/jsonl/toon`.
- **`-o toon` output** — TOON (Token-Oriented Object Notation): same data as `-o json`, ~40% fewer LLM tokens (uniform object arrays become a table); zero-dependency encoder. Every `-o json` command supports it.
- **`-o jsonl` output** — JSON Lines: a top-level array streams one compact object per line, for `jq -c` / `grep` / streaming.
- **`wl agent gaps`** — the reverse of `wl agent ls`: important work with NO session on it (a still-open goal target, or a DOING P0).
- **`wl agent ls` last-worked column** — `52m` / `3d` since each node was last worked + 💤 past `--stale-days` (default 3); `--no-activity` hides it.
- **Overview commands auto-brief in an agent session** — `day` / `ls` / `projects` / `active` / `summary` default to `-q` when a session id is present (17–93% smaller); `--no-brief` opts out.

### Changed
- **Time/date handling consolidated in `timeutil`, enforced by a lint test** — nothing else may read the clock or hand-parse a stored stamp; `parse_ts` / `elapsed_sec` / `shift_ts` / … replace ~26 scattered copies (a live off-by-a-day source).
- **`wl agent ls --by active` no longer counts the bind itself as activity** — a freshly-bound node used to look worked-on forever.

### Fixed
- **A recurring goal target settles by check-in, not status** — `wl tick` now closes a habit / recurrence target in the goal header (`[x]` + `[done/total]`) instead of pinning it `[ ]` forever; `wl day -o json` targets gain a `done` field.
