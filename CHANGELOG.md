...
### Added
- **`wl apply` (wl-diff) — `goal` / `summary` field-ops.** `~ #id` gains `goal <text>` / `summary <text>`, the declarative form of `wl goal set` / `wl recap` (a reserved-tag log). First step of extending apply to cover the full mutation surface (the AI's declarative CRUD).
