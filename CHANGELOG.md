...
- **`wl day` header meta is clearer** — each field gets a distinct marker (🎯 goal · 📝 recap · ⭐ top5 · 📅 week overview); a long/multi-line recap stays quoted (`>`) and soft-wraps on every line instead of breaking flush-left; a goal that names task `#ids` shows ` [done/total]` with a ✅/🟡/⬜ status emoji.
