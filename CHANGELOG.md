...
### Added
- **`wl help` is a short overview now; `wl help --all` lists every topic.** The default `wl help` grew unwieldy as topics piled up, so it now shows just the intro + getting-started + a one-line pointer (`N help topics — wl help --all lists them all`). The full category-grouped index moved behind `wl help --all`; `wl help <topic>` is unchanged.
