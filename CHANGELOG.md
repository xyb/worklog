...
- **`wl query` stale-index hint** — warns on stderr (stdout/JSON stay clean) when the live node count differs from the indexed count, so a node added/changed since the last `wl reindex` (which scores 0.000 on the vector side) is explained instead of silently missing.
