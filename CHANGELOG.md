...
### Changed
- **`log.type` renamed to `log.tag`** — the log-role column is now `tag`, unifying the classification field across all three: node-level `tag` (a label set), `metric.tag` (a datapoint's kind), and `log.tag` (a log's role). One word, one mental model. Migration 0006 renames the column in place (data preserved).
...
- **Metrics fold under their node/log in `wl show` / `wl day` / `wl tree`** — datapoints render indented (`↳ [glucose] 5.4 mmol/L`); in `wl show` an empty `type='metric'` carrier log shows its datapoint directly as a `📊 metric` line (no blank log row); `wl day`/`wl tree` show a node's that-day datapoints under it (the `checkin` marker is skipped — it's reflected by `[x]`); >5 on one node/log are elided (`↳ … N more datapoints`).
- **`node → log → metric` data model** — a new `metric` table holds structured datapoints (check-in / number / measurement) that hang off a log; `log` gains a `type` column (note/goal/summary/clock/…). Foundation for querying/aggregating what was previously stuffed into log text (migration 0002, two-round cross-model reviewed). A metric must hang off a log; `metric.node_id` is denormalized (no FK, trigger-kept consistent); markers store `value_num=1`.
