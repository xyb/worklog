...
The autogenerated `Commits since vX.Y.Z` block on each GitHub Release captures every commit; this file is the human-curated highlight reel — keep entries to one tight line.
...
### Changed
- **`at`/`date` timestamp convention — `*_at` instants stored UTC, `*date` calendar days kept local.** `scheduled_at`→`scheduled_date`, `deadline_at`→`deadline_date`; `$WORKLOG_TZ` pins an offset; day-grouping converts UTC→local. Migration 0007.
...
### Changed
- **Namespace `wl` → `worklog`** (paths + `$WORKLOG_*` env vars; the command stays `wl`).
- **uv replaces venv/pip/requirements** (`pyproject.toml` + `uv.lock`).
- Test fixture clears `XDG_CONFIG_HOME` / `XDG_DATA_HOME`.

### Removed
- `requirements*.txt`; pre-XDG path fallbacks (`~/.worklog/wl.db`).
