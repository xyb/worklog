...
- **`wl migrate-types`** — one-off backfill of the `type.*` namespace onto existing nodes from their kind (idempotent; kind left intact), so `wl ls --para` and friends see pre-existing nodes too.
