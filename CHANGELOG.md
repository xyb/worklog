...
- **`wl show -o json`** — machine-readable output: the full node + all relations (tags, ancestors, props, links, schedule, children, logs, metrics, clock) as one JSON object (an array for several ids), stable DB-column field names, `*_at` UTC / `*_date` local. First step of structured output (`-o`) across commands; lets an AI/script pull an exact field instead of parsing text.
