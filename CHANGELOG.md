...
- **`wl summary` shows the window's goal as a dashboard header** — when scoped to one time node (`--week`/`--month`/`--quarter`/`--year`), it leads with that node's goal + `[done/total]` + target nodes (the `wl day` header, generalized to every level), so a monthly/quarterly review shows its Top5/OKR up top instead of only in `wl show`.
- **`--quarter YYYY-Qn` / `--year YYYY` time-window flags** on `summary`/`logs`/`changes`/`projects` — the shared window parser gained the two coarser grains alongside `--week`/`--month`.

### Fixed
- **`wl day`'s month goal is resolved by the day's own date, not its week ancestry** — a cross-month week (e.g. `W27` hung under June) made its July days surface June's Top5; the month time-node is now looked up by the day's `date.period`, so `wl day` on 07-01…07-05 shows July's goal.
- **Malformed time-window flags no longer crash** — `wl summary --week 2026-07` (a month string given to `--week`) died with a raw `ValueError` traceback; every window flag is now validated through the period oracle and dies with a clear `invalid --week '2026-07' (expected YYYY-Www)` message.
