...
### Added
- **`wl apply` (wl-diff) — `goal` / `summary` / `+metric` field-ops.** `~ #id` gains `goal <text>` / `summary <text>` (the declarative `wl goal set` / `wl recap`, a reserved-tag log) and `+metric tag [value] [unit]` (a node-level datapoint; bare tag = a marker like `wl tick`). `+` add gains an `@metric` sub-line. Part of extending apply to cover the full mutation surface (the AI's declarative CRUD).
