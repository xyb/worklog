...
- **`-o json` machine-readable output** on `wl show` / `wl ls` / `wl logs` — `show` emits the full node + all relations (tags, ancestors, props, links, schedule, children, logs, metrics, clock); `ls` an array of node summaries (filters apply, no display cap); `logs` an array of log rows. Stable DB-column field names, `*_at` UTC / `*_date` local, empty result → `[]`. Commands without json support reject `-o` rather than silently printing text. Lets an AI/script pull an exact field instead of parsing the text view.
