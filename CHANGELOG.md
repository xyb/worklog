...
### Added
- **`wl apply` (wl-diff) — `goal` / `summary` / `+metric` / `sched` / `recur` field-ops.** `~ #id` gains `goal <text>` / `summary <text>` (the declarative `wl goal set` / `wl recap`, a reserved-tag log), `+metric tag [value] [unit]` (a node-level datapoint; bare tag = a marker like `wl tick`), and `sched <date>` / `recur <rule>` / `-sched` (a PRECISE sched-table row / rrule — the declarative `wl sched`; distinct from `scheduled`, which only sets the rough `scheduled_date` column). `+` add gains an `@metric` sub-line. Part of extending apply to cover the full mutation surface (the AI's declarative CRUD).
