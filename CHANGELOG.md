...
### Breaking
- **`kind` retired — classification is now the orthogonal `type.*` prop namespace** (`type.para` area/project/task · `type.date` level · `type.habit` · `type.meetlog`); a node can hold several at once. Migration **0011** backfills + verifies a lossless round-trip + drops the `node.kind` column, auto-run on first use under a write lock with pre/post backups.
