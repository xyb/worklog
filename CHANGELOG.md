...
- **Newcomer-friendlier `--help`.** Top-level `wl -h` now opens with a one-line what-it-is +
  ...
  work / time / see it / organize / plan-reflect / bulk-setup) plus a "good to know" tips block
  (id forms, fuzzy dates, planned/DOING behavior, completion, shortcuts). `wl add -h` gained
  help text on every option (`title`, `-p` = A/B/C priority, …) and leads with simple,
  relatable examples instead of work-jargon.
