...
### Added
- **`wl show` folds metrics under their log** — a log's datapoints render indented beneath it (`↳ [glucose] 5.4 mmol/L`); an empty `type='metric'` carrier log shows its datapoint directly as a `📊 metric` line (no blank log row); >5 metrics on one log are elided (`↳ … N more datapoints`).
