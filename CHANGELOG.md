...
### Fixed
- **`wl agent ls` / `wl agent` titles truncate cleanly to one line.** They used a fixed 50-character cut that ignored CJK width (2 cols/char) and the row prefix, so a long title overflowed the terminal and wrapped. Now budgeted against the real prefix width (display-width aware) — each row is exactly one line ending in `…`.
