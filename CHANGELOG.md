...
### Added
- **`wl log show #L282`** — print one log entry's full, untruncated content by its log id (the list views truncate each log to one line). Accepts `#L282` / `L282` / `282`.
- **`wl retag #L282 <tag>`** — change a log's tag directly (goal / summary / a custom marker); `note` / `none` / `-` / empty clears it back to a plain note.
- **`wl ls --root <id>`** — flat list of a node's whole subtree (all descendants, recursive), vs `--parent` (direct children only). Mirrors `wl tree --root`.
- **`wl metric ls --tag X` with the node omitted** — search a metric tag across *every* node (each row prefixed with its node id), to locate all uses before renaming. Node-scoped `wl metric ls <id>` unchanged.
- **`wl help body`** — a topic explaining the two `body` fields (node.body long description vs log.body per-entry text) and which command writes which.
- **`wl recap --diff [--date D]`** — list the plain-note logs added that day *after* the recap was written (what `wl day`'s "⚠ N change(s) after recap" counts; excludes checkin/metric noise), so you can judge whether to rewrite the recap.

### Changed
- **`wl relation <id> <ids…>` defaults to `related`** — omit the type word (`wl relation 42 7 9` = `… related 7 9`); `split-from`/`split-into` still explicit.

### Fixed
- **`wl show` multi-line `body`** — continuation lines now indent under the value instead of starting flush-left at column 0.
- **`wl query` Chinese recall** — a node matching *every* query term (an exact full match) now leads the results, ordered by keyword field-weight, instead of being buried under noisy semantic neighbors; the vector list is also capped before RRF so its long tail can't dilute precise hits (e.g. `wl query 微信导出` now surfaces a node titled "微信全量导出" first).
- **`wl query` title highlight** — highlights each segmented term separately, so a non-contiguous match (terms split by other words, e.g. 微信…导出) lights up the matched parts instead of nothing.
- **`wl query` stale-index hint** — when some live nodes aren't in the index yet, warns on stderr (stdout/JSON stay clean) with just the count: `⚠ N node(s) not indexed — run wl reindex`, so a node added since the last reindex (which scores 0.000 on the vector side, carried only by keyword) is explained instead of silently missing.
