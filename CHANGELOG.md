...
### Added
- **Signed relative-delta dates** — `+1` / `-2d` / `+3w` / `+1m` / `-1y` (signed number + optional unit, resolved from today) on every date-accepting command (`wl day -2`, `wl sched 42 +3w`, `wl log … --date -1d`). Month/year deltas clamp the day; `_WlParser` lets a negative delta work as a positional on 3.9–3.13.
- **Relative date words resolve everywhere, connector- and case-insensitive** — `next-week`/`-month`/`-quarter` → that period's first day, accepted by `wl sched`/`wl day`; `NEXT_WEEK` / `next week` and plurals (`+2weeks`) parse. `someday` stays `defer`-only.
...
### Changed
- **`wl set` / `wl prop set` / import `props` refuse reserved node-field names** (`status`/`priority`/`title`/…) so a prop can't shadow a real column (it had produced two `status` values in `wl show`); each rejection points to the right editor.
