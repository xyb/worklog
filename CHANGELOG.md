...
### Added
- **`-o json` is now a global flag** — place before the verb (`wl -o json ls`) or after (`wl ls -o json`); both work for every command. Postfix syntax is per-command via `parents=[output_parent]`; prefix sets it on the root parser with `argparse.SUPPRESS` on the per-command default so the two don't conflict.
- **JSON output for all remaining commands** — `find`, `focus`, `ancestors`, `descendants`, `agenda`, `changes`, `goal` (read + ls), `prop ls`, `log ls`, `log show`, `link ls`, `tag ls`, `sched ls`, `metric ls`, `add`, `log` (write), `done`, `cancel`, `reopen`, `wait`, `defer`, `start`, `stop`, `active`, `relation` (read + write), `recap`, `clock ls`, `clock edit`, `clock rm`, `agent` (show + ls), `date ls`, `spent`, `unlink`, `set`, `unset`, `tag add/rm`, `metric add/edit/rm`, `sched add/rm`, `prop set/rm`. Write commands suppress their human-readable confirmation lines in json mode so stdout stays machine-parseable.
- **`wl help output`** — new help topic covering the global `-o json` flag, accepted syntax, return shapes, and examples for every command; linked from `show`, `ls`, `query`, `logs`.
