...
- **查询拆解——简单读 + Python 组合，优先于复杂 SQL。** 既定方向，和 G3、以及"不用外键" + "时间模型用 Python 从 date 推导归属、而非 parent 链"一脉相承: 把一个复杂读拆成若干简单单表 SELECT、在 Python 里组合——每个封装成小函数，对被多次查询的分类加一个命令周期的缓存——而不是把 JOIN / EXISTS 子查询 / 聚合塞进一条 SQL。**原因:** 库很小（几千行量级、~2 MB），全表读 + Python 组合是亚毫秒级，实测比它替代的子查询 SQL **更快**（`workitem_sql` 过滤——6 个 EXISTS 子查询——1.90 ms/次；读全部 node 再用纯函数 `node_type_from_props` 在 Python 里分类是 1.59 ms，结果集完全一致）。复杂 SQL 也是最难维护、最难做性能剖析、最难（被人或 AI）重构的部分，所以减少它是可维护性的净收益，不是性能代价。`db_table` 那句"NOT an ORM——复杂读保留显式 SQL"相应收窄: 只在拆解确实帮不上的地方（如 GROUP BY 词表统计）或投影场景保留 raw SQL；凡是能靠 prop 分类的都搬到 Python 索引。仍在手写 SQL 的简单单表读（`semantic.py` 的关键词 LIKE、count(*)）已对齐到 `_db.query` / `Node.count`——`cmd_find` 本就是模板。
- **type 分类是 Python 快照，不是 EXISTS 子查询。** 节点的 type 由它的 `type.*` props 经纯函数 `node_type_from_props(props)` 推导（优先级 para > date > habit > meetlog > custom > task）。把这套重新编码成 EXISTS 子查询的 SQL 形式（`workitem_sql`、`nodes_with_type`、`time_node_by_period`、逐节点的 `node_type`）正在迁到一个命令周期的索引上: 一次性把所有存活的 `type.*` props 读进 `{node_id: {key: value}}`，再用 dict 查找分类。这个索引是**快照**——在读命令入口建一次，命令内所有分类复用，不跨写入持有。
