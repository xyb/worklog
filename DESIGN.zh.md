...
- **`sched` 表**：`(id, node_id, on_date, recurrence, created_at)`。`on_date` 与 `recurrence` 二选一；一个任务可有多条（多天 / 一次性+重复并存）。
  - `on_date`：一次性排到具体某天 `YYYY-MM-DD`。
  - `recurrence`：怎么重复——`daily` / `weekly:Mon,Wed` / `monthly:5|-1` / `quarterly:M-D|-1` / `yearly:MM-DD|-1`（`_normalize_recurrence` 校验，星期归一化成 Mon..Sun）。
  - **这是自造 DSL，不是 RFC 5545 RRULE**，尽管这个字段在迁移 0014 之前一直叫 `rrule`。有两处按标准根本读不通：`quarterly:` 不是 RFC 的频率（标准表达季度是 `FREQ=MONTHLY;INTERVAL=3`），而 `weekly:-1` 里的 `-1` 表示**周日**、标准里的 `-1` 前缀却表示**该周期的最后一个**。改名是因为懂 RRULE 的读者恰恰会被旧名字误导；语法保持简短是有意的（G3——这些字符串天天要敲要读）。
  - **每条规则都是目标日期的纯谓词**——`_sched_fires` 只看 `target` 那一天就能回答「这天触发吗」，不需要锚点、不需要历史。这正是重复规则得以无状态、幂等的原因。但要清楚代价：**间隔类规则（「每隔一周的周三」）在这个模型里表达不了**，因为间隔需要一个相位起点。要支持它，得给 recurrence 行加锚点日期（相当于标准的 `DTSTART`），不是加个关键字就行。用 ISO 周奇偶取巧不能替代——53 周的年份（2026 就是）跨年时奇偶会翻转，相位会永久偏移一周。
- **`_sched_fires(on_date, recurrence, target)`**：该行是否在 target 当天触发。`_scheduled_node_ids(con, target)` 收集当天被命中的 node。
- **`wl sched <id> [when] [--recur R] [--clear]`**：排期 / 重复 / 清除 / 无参列出。`when` 走 `_resolve_concrete_date`（YYYY-MM-DD / 今天 / 明天 / 后天）。
- **结束循环——`wl sched stop <id> [date] [--rule R]`**：在一个**含当天**的结束日（默认今天）停止循环，保留历史。以 recurrence 字符串上的 `;until=YYYY-MM-DD` 后缀编码（`weekly:Wed;until=2026-06-30`），**不**加新列：`_split_until(recurrence)` 剥离后缀，`_sched_fires` 对 `target > until` 返回 False——所以每个 recurrence 消费点（`_next_sched_fire`、`wl show`）先剥离就天然 until 感知，改动面最小。对比 `--clear`/`rm` 会删掉整行、连**过去**的日子也抹掉。一个 node 挂多条 recurrence 时默认全停，`--rule` 指定其一。`wl show` 渲染成 `recur weekly:Wed (stopped 2026-06-30)`。`_recurrence_is_active(recurrence, today)`（无 until，或 until >= today）是「这条循环是否还活着」的单一判据，`wl done` 循环警告、`wl show` 的 `last check-in:` 行、`wl ls --not-checked-in` 共用。
- **`wl agent` 可观测性——正向视图与反向视图。** `wl agent ls` 列出活着的 session↔node 绑定，每行带一个**读时派生**的活跃列：节点上一次被写 log 到现在有多久（`_age_short`），超过 `--stale-days`（默认 `AGENT_STALE_DAYS` = 3）再加个 💤。一律不落库——活跃度是**时变状态**，存下来的那一刻就开始变错。计算「上次干活时间」时**排除绑定动作自己写的那条 history log**：把它算进去，刚绑定的节点就会永远显示活跃，而这恰恰是本视图要检验的那个说法。`wl agent gaps` 是反向视图，也是正向列表结构上答不了的那个问题（「什么事情没有任何人在推？」）：重要但无绑定的工作，两个来源——**今日 goal 里仍未了结的 target**（已了结的由 `_target_settled` 滤掉，所以打过卡的循环目标不会被误报）和 **DOING 的 P0**（声称在做、却没有 session 在推）。两条排除规则都是真实数据教出来的：**循环任务**永远离不开 DOING（`wl tick` 不动 status），它靠打卡维持而不是靠 session，所以跳过；而**仅仅是开着的 P0 不算缺口**——优先级是长期排序，不是「正在推进」的声明，把它们算进来等于一次捞出几乎所有开着的 P0，一个永远在响的告警等于从不告警。
...
`-1` 在所有 recurrence 里语义统一:**当前周期的最后一天**。短月份自动处理(`monthly:31` 2 月不触发; `quarterly:3-31` 6 月不触发因为没 31)。

实现走 `_sched_fires(on_date, recurrence, target)`(target 日期返回 True/False),`_normalize_recurrence` 在写入时校验范围 + 数字 → Mon..Sun 归一化。`_scheduled_node_ids(con, target)` 遍历 sched 表算 target 当天命中的 node id, 给 `wl day` / `wl checkin` 用。
