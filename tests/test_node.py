...
class TestNodeGroupShortcutEquivalence:
    def test_node_add_equals_add(self, cli, tmp_db):
        cli("node", "add", "via group")
        cli("add", "via shortcut")
...
class TestNodeEdit:
    ...
    def test_edit_body_deadline_and_set_habit(self, cli, tmp_db):
        cli("add", "t")
        cli("node", "edit", "1", "--body", "b", "--deadline", "2026-06-30")
        cli("set", "1", "type.habit", "true")   # classification now via the type.* prop, not -k
        con = tmp_db.db_connect()
...
class TestNodeReparent:
    def test_reparent_moves(self, cli, tmp_db):
        cli("add", "parent", "--para", "project")   # 1
        cli("add", "child")        # 2 (top-level)
    ...
    def test_reparent_cycle_refused(self, cli):
        ...
        cli("add", "b", "--parent", "1")  # 2 (child of 1)
...
class TestNodeReviewFixes:
    ...
    def test_rm_cascades_through_tombstoned_intermediate(self, cli, tmp_db):
        ...
        cli("add", "A", "--para", "project")                  # 1
        cli("add", "B", "--para", "project", "--parent", "1")  # 2
        cli("add", "C", "--parent", "2")     # 3
        con = tmp_db.db_connect()
