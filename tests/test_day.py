...
class TestDay:
    ...
    def _seed(self, cli, date="2026-05-28"):
        ...
        cli("add", "2026-05", "--prop", "type.date=month", "--parent", "1")                     # 2
        cli("add", "biz aggregation", "--para", "project", "-t", "work", "--parent", "2")     # 3
        cli("add", "czbooks", "--para", "project", "-t", "personal", "--parent", "2")  # 4
        cli("add", "agg taskA", "-p", "A", "-t", "work", "--parent", "3")     # 5
        cli("add", "crawler taskB", "-p", "C", "-t", "personal", "--parent", "4")  # 6
        cli("done", "5")
    ...
    def test_day_by_plan(self, cli):
        ...
        cli("add", "planned task", "-t", "work,planned", "--parent", "2")    # 3
        cli("add", "temp task", "-t", "work,unplanned", "--parent", "2")  # 4
    ...
    def test_day_by_plan_unscheduled_is_unplanned(self, cli):
        ...
        cli("add", "2026", "--prop", "type.date=year")
        cli("add", "proj", "--para", "project", "-t", "work", "--parent", "1")
        cli("add", "some task", "-t", "work", "--parent", "2")  # 3
...
class TestDayMeta:
    ...
    def _seed_day(self, cli):
        cli("add", "2026", "--prop", "type.date=year", "--prop", "date.period=2026")                                     # 1
        cli("add", "2026-05", "--prop", "type.date=month", "--prop", "date.period=2026-05", "--parent", "1")                # 2
        cli("add", "2026-05-20", "--prop", "type.date=day", "--prop", "date.period=2026-05-20", "--parent", "2")               # 3
        cli("add", "t", "-t", "work")                          # 4
...
class TestDayMetaRendering:
    ...
    def _setup_day_with_props(self, cli, **props):
        ...
        cli("add", today, "--prop", "type.date=day", "--prop", f"date.period={today}")
    ...
    def test_day_renders_week_goal(self, cli):
        ...
        cli("add", "2026-W22", "--prop", "type.date=week", "--prop", "date.period=2026-W22")  # id 1
        cli("add", today, "--prop", "type.date=day", "--prop", f"date.period={today}", "--parent", "1")  # id 2
...
class TestDayMetaMarkersAndGoalProgress:
    ...
    def _day(self, cli, **props):
        from datetime import date
        today = date.today().isoformat()
        cli("add", today, "--prop", "type.date=day", "--prop", f"date.period={today}")        # id 1
        for k, v in props.items():
            cli("set", "1", k, v)
        return today
    ...
    def _day_after(self, cli):
        from datetime import date
        today = date.today().isoformat()
        code, out, _ = cli("add", today, "--prop", "type.date=day", "--prop", f"date.period={today}")
        import re
...
class TestDayJson:
    def test_day_json_structure(self, cli):
        from datetime import date
        today = date.today().isoformat()
        cli("add", today, "--prop", "type.date=day", "--prop", f"date.period={today}")                      # 1
        cli("add", "task a", "-t", "work")     # 2
        cli("sched", "2", "today")
        cli("log", "2", "did it")
        cli("set", "1", "goal", "do #2")
        import json
        ...
        # WL#765/#901: day tasks carry the orthogonal type facet, no collapsed kind
