"""End-to-end sample scenario (extracted from the original test_wl.py monolith)."""
import sqlite3
import pytest


class TestFullSampleScenario:
    """Simulate a realistic flow: time hierarchy + project + task + log + done + link."""
    ...
    def test_full_sample_scenario(self, cli, tmp_db):
        ...
        cli("log", "8", "5/19 09:42 broke down requirements: export_for_ai path")
        cli("log", "8", "5/20 14:55 end-to-end works; accuracy 6/6 -> 7/7")
        cli("log", "8", "5/21 11:08 wrap-up: 87% cost reduction")
        # link to vault docs
        ...
        assert "rewrite ingestion pipeline" in out
        assert "DONE" in out
        ...
        assert "rewrite ingestion pipeline" in tree
